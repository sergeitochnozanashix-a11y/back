...
* **`railway service status` тоже показывает последний УСПЕШНЫЙ деплой.** Пока
  новый собирается, команда отдаёт SUCCESS от предыдущего - цикл ожидания
  вида `until status = SUCCESS` завершается мгновенно, и проверять вы будете
  старый код. Ждать надо по самому свежему деплою:
  ```bash
  until railway deployment list -s backend | sed -n '2p' | grep -qE 'SUCCESS|FAILED'; do sleep 20; done
  ```
